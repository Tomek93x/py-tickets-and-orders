...
def actors_data():
    ...
    Actor.objects.create(name="Scarlett Johansson")
    Actor.objects.create(name="George Clooney")


@pytest.fixture()
def movies_data(genres_data, actors_data):
    keanu = Actor.objects.get(name="Keanu Reeves")
    scarlett = Actor.objects.get(name="Scarlett Johansson")
    george = Actor.objects.get(name="George Clooney")

    matrix = Movie.objects.create(title="Matrix", description="Matrix movie")
    matrix.actors.add(keanu, scarlett)
    matrix.genres.add(Genre.objects.get(name="Action"))

    matrix2 = Movie.objects.create(title="Matrix 2", description="Matrix 2 movie")
    matrix2.actors.add(scarlett)
    matrix2.genres.add(Genre.objects.get(name="Action"))

    batman = Movie.objects.create(title="Batman", description="Batman movie")
    batman.actors.add(george)
    batman.genres.add(Genre.objects.get(name="Drama"))

    titanic = Movie.objects.create(title="Titanic", description="Titanic movie")
    titanic.genres.add(Genre.objects.get(name="Action"))
    titanic.genres.add(Genre.objects.get(name="Drama"))

    good_bad = Movie.objects.create(
        title="The Good, the Bad and the Ugly",
        description="The Good, the Bad and the Ugly movie",
    )
    good_bad.genres.add(Genre.objects.get(name="Western"))

    Movie.objects.create(title="Harry Potter 1", description="HP1")
    Movie.objects.create(title="Harry Potter 2", description="HP2")
    Movie.objects.create(title="Harry Potter 3", description="HP3")
    Movie.objects.create(title="Harry Kasparov: Documentary", description="Doc")
...
def movie_sessions_data(movies_data, cinema_halls_data):
    MovieSession.objects.create(
        show_time="2019-08-19 20:30",
        cinema_hall_id=1,
        movie_id=1
    )
    MovieSession.objects.create(
        show_time="2017-08-19 11:10",
        cinema_hall_id=3,
        movie_id=4
    )
    MovieSession.objects.create(
        show_time="2021-04-03 13:50",
        cinema_hall_id=2,
        movie_id=5
    )
    MovieSession.objects.create(
        show_time="2021-04-03 16:30",
        cinema_hall_id=3,
        movie_id=1
    )
...
def test_movie_service_get_movies_with_full_data(movies_data):
    assert list(get_movies(genres_ids=[1, 2], actors_ids=[2, 3], title="matrix").values_list("title")) == [("Matrix",), ("Matrix 2",)]
    assert list(get_movies(genres_ids=[1, 2], actors_ids=[2, 3], title="batman").values_list("title")) == [("Batman",)]
...
def test_user_service_create_user():
    ...
    assert get_user_model().objects.get(username="User1").password != "Password1234"
    assert get_user_model().objects.get(username="User2").password != "Password5678"
...
def test_user_service_update_user_with_no_data(users_data):
    ...
    assert list(get_user_model().objects.filter(id=1).values_list("username", "first_name", "last_name", "email")) == [
        ("user1", "", "", ""),
    ]
...
def test_order_service_create_order_without_date(create_order_data, tickets):
    ...
    assert list(Ticket.objects.filter(movie_session=1).values_list("row", "seat", "movie_session")) == [
        (10, 8, 1),
        (10, 9, 1)
    ]
...
def test_ticket_clean_row_out_of_range(movie_sessions_data, orders_data):
    ...
    assert str(e_info.value) == "{'row': ['row number must be in available range: (1, 10)']}"
...
def test_ticket_clean_seat_out_of_range(movie_sessions_data, orders_data):
    ...
    assert str(e_info.value) == "{'seat': ['seat number must be in available range: (1, 12)']}"
